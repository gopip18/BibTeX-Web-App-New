...
def get_bibtex(reference):
    ...
    try:
        response = openai.ChatCompletion.create(
            model="gpt-3.5-turbo",  # Using GPT-3.5 Turbo for efficient processing
            messages=[
                {"role": "system", "content": "You are an assistant trained to convert academic references into BibTeX format."},
                {"role": "user", "content": prompt_text}
            ],
            temperature=0.3,  # Lower temperature to ensure more predictable and consistent outputs
            max_tokens=250    # Adjust based on typical length of BibTeX entries; increase if entries are cut off
        )
