...
# Function to trim the conversation to avoid exceeding token limits (only trim when necessary)
...
def main_page():
    ...
    if user_prompt:
        ...
        # Trim conversation only if it exceeds the token limit
        st.session_state.chat_history_main = trim_conversation(st.session_state.chat_history_main)

        # Send the conversation to GPT-3.5-turbo and get a response
        setup_openai()
        try:
